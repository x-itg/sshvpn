# ssh_vpn
# 这里提供一种翻墙新思路，解决上网问题
- 本地windows电脑安装贝锐蒲公英异地组网软件和git（带ssh客户端）；香港linux（ubuntu）服务器安装贝锐蒲公英异地组网软件；本地windows使用ssh指令使用本地指定端口连接蒲公英局域网内的香港linux服务器ip；本地windows设置socks指定端口代理上网
## 1、组网代理
...
## 2、SSH 无密码登录服务器
- windows命令行运行：ssh-keygen -t rsa
- windows命令行到.ssh文件夹：cd .ssh
- windows命令行上传公钥文件到linux服务器：scp id_rsa.pub ubuntu@172.16.0.198:~/.ssh/authorized_keys
- windows命令行无密码登录linux服务器：ssh ubuntu@172.16.0.198
